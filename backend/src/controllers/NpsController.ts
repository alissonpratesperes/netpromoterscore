...
import { SurveysUsersRepository } from "../repositories/SurveysUsersRepository";
    ...
    class NpsController {
        async execute(request: Request, response: Response) {
            ...
            const calculate = Number((((detractor - promoter) / totalAnswers) * 100).toFixed(2));

                return response.status(200).json({ detractor, passive, promoter, totalAnswers, nps: calculate });
        };
    };
